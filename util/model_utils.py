...
def urlretrieve(remote_path, weight_path, progress_print):
    ...
    try:
        ...
        logger.info(Fore.RED + f'SSLError detected, so try to download without ssl')
...
def check_and_download_models(weight_path, model_path, remote_path):
    ...
    if not os.path.exists(weight_path):
        logger.info(Fore.GREEN + f'Downloading onnx file... (save path: {weight_path})')
        urlretrieve(
    ...
    if model_path!=None and not os.path.exists(model_path):
        logger.info(Fore.GREEN + f'Downloading prototxt file... (save path: {model_path})')
        urlretrieve(
